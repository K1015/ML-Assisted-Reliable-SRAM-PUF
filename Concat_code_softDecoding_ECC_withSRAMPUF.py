...
path = ''
GoldenResp = np.load(path+'GResp_temp_all_May20.npy')
CorrectedRespSpecific = np.load(path+'Corrected_Resp_specific_May20.npy')
CorrectedResp = np.load(path+'CorrectedResp_May20.npy')
Reliability_PUF = np.load(path+'Reliability_PUF_all.npy')
...
for board in range(BoardTotal):
    MR = np.where(Reliability_PUF[board,refloc,:,:]>0.9)
    ...
    for predTemp in TempRange:  
        ...
        soft_info_decoding = soft_info[board,tidx,MR[0],MR[1]].reshape(-1)
