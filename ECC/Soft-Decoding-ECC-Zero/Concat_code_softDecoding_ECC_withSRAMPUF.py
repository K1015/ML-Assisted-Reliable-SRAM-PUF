...
CorrectedRespSpecific = np.load(path+'CorrectedResp_Zero_Temp_ContinualLearning.npy')
CorrectedResp = np.load(path+'CorrectedResp_Zero_Temp_TransferLearning.npy')
...
for board in range(BoardTotal):
    MR = np.where(Reliability_PUF[board,:,:]>0)
...
#%%
np.save('BER_Accuracy_Vanilla_softconc_RM(r%d,m%d)_Rep(%d).npy'%(r,m,n2),BER_Accuracy_Vanilla)
np.save('BER_Accuracy_PostECC_softconc_RM(r%d,m%d)_Rep(%d).npy'%(r,m,n2),BER_Accuracy_PostECC)
np.save('BER_Accuracy_PostML_ECC_sp_softconc_RM(r%d,m%d)_Rep(%d).npy'%(r,m,n2),BER_Accuracy_PostML_ECC_sp)
np.save('BER_Accuracy_PostML_ECC_softconc_RM(r%d,m%d)_Rep(%d).npy'%(r,m,n2),BER_Accuracy_PostML_ECC) 

#%%
BER_Accuracy_Vanilla = np.load('BER_Accuracy_Vanilla_softconc_RM(r%d,m%d)_Rep(%d).npy'%(r,m,n2))
BER_Accuracy_PostECC = np.load('BER_Accuracy_PostECC_softconc_RM(r%d,m%d)_Rep(%d).npy'%(r,m,n2))
BER_Accuracy_PostML_ECC_sp = np.load('BER_Accuracy_PostML_ECC_sp_softconc_RM(r%d,m%d)_Rep(%d).npy'%(r,m,n2))
BER_Accuracy_PostML_ECC = np.load('BER_Accuracy_PostML_ECC_softconc_RM(r%d,m%d)_Rep(%d).npy'%(r,m,n2))   
#%%
#For Printing
boardID = 5
...
#plt.yticks(ticks=np.array([5e-2,1e-1,5e-1,1e-1,1,5,10,15,20]))
plt.ylabel(r'BER (%) @ 70$^{\circ}$C',fontsize=42,fontdict=font)
